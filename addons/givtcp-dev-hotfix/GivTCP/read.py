...
async def watch_plant(
        ...
        try:
            ...
            tmp="/config/GivTCP/"+GiV_Settings.serial_number+"_caps.pkl"
            logger.critical("Checking for cached capabilities file: "+str(tmp))
            if exists(tmp):
                with open(tmp, 'rb') as inp:
                    caps=pickle.load(inp)
                logger.critical("Skipping Detect - using cached capabilities")    
...
###    inv=GEInv.getall()
    ...
    if Meters:
        for m in Meters:
            meters['Meter_ID_'+str(m)]=getall(Meters[m])
        raw['meters']=meters
    ...
    try:
        ...
        if not isHV:
            ...
            for num,stack in enumerate(HVStack):
                ...
                bcudata['Stack_Design_Capacity']=round((bcu.battery_nominal_capacity_ah*bcu.number_of_modules)*0.9,2)     # Usable kWh is 10% less than actual
                bcudata['Stack_SOC_kWh']=round((bcu.remaining_battery_capacity_ah*bcu.number_of_modules)*0.9,2)     # Usable kWh is 10% less than actual
                ...
                for b in stack.bmus:
                    if b.is_valid():
                        ...
                        sn=b.serial_number
                    if sn and sn.upper().isupper():          # Check for empty battery object responses and only process if they are complete (have a serial number)
                        logger.debug("Building battery output: ")
                        battery = {}
                        ...
                        if is3PH:
                            for i in range(24):
                                battery['Battery_Cell_'+str(i+1)+'_Temperature'] = getattr(b,'t_cell_'+str(i+1).zfill(2))
                                totaltemp=totaltemp+getattr(b,'t_cell_'+str(i+1).zfill(2))
...
def runAll2(plant: Plant):  # Read from Inverter put in cache and publish
    ...
    try:
        ...
        missing_key = e.args[0] if e.args else None
        logger.error("Key Error getting Battery Data: " + (f"Missing key {missing_key!r}") )
        return ("runAll2 Error processing registers: Missing key " + repr(missing_key))
    except Exception:
        e=sys.exc_info()[0].__name__, os.path.basename(sys.exc_info()[2].tb_frame.f_code.co_filename), sys.exc_info()[2].tb_lineno
