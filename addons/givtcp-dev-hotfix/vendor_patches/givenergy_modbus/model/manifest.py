...
# Legacy per-slot Charge/Discharge Target SOC registers (the third register in each
# HR240-299 extended-slot triple, alongside the start/end pair WRITE_SAFE_SINGLE_PHASE
# already admits above). All 20 are app-confirmed (pdu.write_registers.WRITE_SAFE_REGISTERS
# / docs/reference/registers/app_4.0.7_inventory.json) — the original #48 sweep only
# picked up HR299 ("DC Discharge 10 Lower SOC % Limit"); re-auditing the inventory
# directly against this register family surfaced the other 19 as equally present
# (app labels "AC/DC Charge/Discharge N Upper/Lower SOC % Limit"), an omission in the
# original sweep rather than a gap in evidence.
#
# Still its own set rather than folded into WRITE_SAFE_SINGLE_PHASE or gated on
# has_extended_slots generally: the app inventory isn't per-model scoped (it's the
# union of every control the app exposes across its supported models, not a
# confirmation that every extended-slot model supports every listed register), and
# this specific family was cross-checked against Model.HYBRID_HV_GEN3 in particular
# (the inventory's own model_codes maps 0x8103 → "GIV-HY-10.0-G3-HV 10KW", i.e. this
# model) — corroborated by a pre-rewrite vendored client (givenergy_modbus_async, no
# write-safety gating at all) having used this exact register family against
# HYBRID_HV_GEN3 hardware for an extended period, per the site owner's account (see
# Texelo/giv_tcp's hotfix11/hotfix12 addon README). Other extended-slot models
# (HYBRID_GEN4, ALL_IN_ONE, ALL_IN_ONE_HYBRID) get no claim here pending their own
# check against the inventory — narrow and reviewable independently rather than
# assumed transitively from "same register layout."
WRITE_SAFE_HYBRID_HV_GEN3_LEGACY_TARGETS: frozenset[int] = frozenset(
    {
        *range(242, 270, 3),  # CHARGE_TARGET_SOC_1..10 (242, 245, 248, ..., 269)
        *range(272, 300, 3),  # DISCHARGE_TARGET_SOC_1..10 (272, 275, 278, ..., 299)
    }
)
...
def write_safe_registers(model: Model | None, arm_fw: int | None = None) -> frozenset[int]:
    ...
    if model == Model.HYBRID_HV_GEN3:
        safe = safe | WRITE_SAFE_HYBRID_HV_GEN3_LEGACY_TARGETS
    return safe
