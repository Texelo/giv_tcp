...
WRITE_SAFE_REGISTERS = {
    ...
    # Legacy per-slot Charge/Discharge Target SOC, HR240-299's extended-slot layout
    # (app labels: "AC Charge N Upper SOC % Limit" / "DC Discharge N Lower SOC %
    # Limit"). Only HR299 had been added prior to this pass (#48's original sweep);
    # re-auditing docs/reference/registers/app_4.0.7_inventory.json directly against
    # this register family surfaced the other 19 as equally present in the app's own
    # inventory — an omission in the original sweep, not a difference in evidence
    # tier. model_codes in that inventory confirms 0x8103 = "GIV-HY-10.0-G3-HV 10KW",
    # i.e. HYBRID_HV_GEN3, the model this surfaced against (see
    # WRITE_SAFE_HYBRID_HV_GEN3_LEGACY_TARGETS in manifest.py — Gate 1 restricts
    # actual write access to that model; the app inventory itself isn't per-model
    # scoped, so Gate 2 admits the whole family here as it does every other
    # app-confirmed register).
    242,  # CHARGE_TARGET_SOC_1 (app: "AC Charge 1 Upper SOC % Limit")
    ...
    272,  # DISCHARGE_TARGET_SOC_1 (app: "DC Discharge 1 Lower SOC % Limit")
